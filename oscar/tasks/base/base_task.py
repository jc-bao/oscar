...
class BaseTask:
  ...
  def __init__(
    ...
    self.n_actors = None
    self.envs = []
    # Actor IDs corresponding to actors that have actuatable joints
    self.actors_with_dof = []
    # Utility counter, e.g.: keeping track of frames when saving video
    self._i = 0
    ...
    self.extras = {}
    # Holds keyword-mapped contact force magnitudes for relevant bodies
    self.contact_forces = {}
    ...
    self.sim_params = self.sim_cfg.get(
      "sim_params", get_sim_params(self.sim_cfg))
  ...
  def create_sim(self):
    self.sim = self.gym.create_sim(
      self.compute_device, self.graphics_device, self.physics_engine, self.sim_params)
  ...
  def _check_contact(self, groupA, groupB, mode="any"):
    ...
    n_B = len(groupB)
    n_AB = n_A * n_B
    contact_mapping = {(A, B): i * n_B + j for i, A in enumerate(groupA)
    ...
    contacts_info_tensor = torch.zeros(
      self.n_envs, n_AB + 1, device=self.device)

    # Populate info tensor
    for c in contacts:
      if -1 < c['env0'] < self.n_envs:
        contacts_info_tensor[c['env0'], contact_mapping.get(
          (c['body0'], c['body1']), -1)] = 1
        contacts_info_tensor[c['env0'], contact_mapping.get(
          (c['body1'], c['body0']), -1)] = 1
    ...
    if mode == "any":
      ...
      raise ValueError(
        f"Invalid mode specified. Options are 'any' or 'all'; got: {mode}")
  ...
  def compute_observations(self):
    ...
    self.obs_buf = obs_buf * \
      (1.0 + self.observation_noise * (-1.0 + 2.0 * torch.randn_like(obs_buf)))
  ...
  def compute_resets(self):
    ...
    self.reset_buf[:] = torch.where(
      self._last_reset_buf > 0, torch.zeros_like(reset_buf), reset_buf)
