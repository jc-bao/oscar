...
class Pick(AgentTask):
  ...
  def __init__(
    ...
    # Maximum number of environments allowable for using path segment visualization
    self.max_envs_for_vis = 16
    ...
    assert self._path_shape in {
      "straight", "bend1"}, f"Invalid path shape specified: {self._path_shape}"
    if self._path_shape == "straight":
      self._n_segments = self._path_segment_bounds[0]
    else:       # bend1
      self._bend_size = task_cfg.get("path_kwargs", {}).get(
        "bend_size", 2)                   # Hardcoded bend size for now
    ...
    self._start_platform_surface_pos = None
    # (x,y,z) position of goal platform state
    self._goal_platform_surface_pos = None
    ...
    # Actor IDs corresponding to path segments for a given env
    self._path_segment_ids = None
    # Integer ID corresponding to the current path segment to be reached for a given env
    self._current_path_segment = None
    # Integer ID corresponding to the (potentially outdated) last path segment ot be reached for a given env
    self._last_path_segment = None
    # Boolean, whether we're using colored visualization of the path progress or not
    self._use_path_visualization = None
  ...
  def _create_envs(self):
    ...
    platform_asset = self.gym.create_box(
      self.sim, *[self._platform_depth,
                  self._path_bounds[1], self._thickness], platform_opts
    )
    ...
    table_stand_pos = [-0.60, 0.0, 1.0 + self._thickness /
    ...
    path_segment_asset = self.gym.create_box(
      self.sim, *[self._path_width, self._path_width, self._thickness], path_segment_opts)

    # Define start pose for agent
    agent_start_pose = gymapi.Transform()
    agent_start_pose.p = gymapi.Vec3(-0.55, 0.0,
                                     1.0 + self._thickness / 2 + table_stand_offset)
    ...
    self._start_platform_surface_pos = np.array(
      start_platform_pos) + np.array([0, 0, self._thickness / 2])

    # Define start pose for goal platform
    goal_platform_pos = start_platform_pos + self._path_bounds[0] * np.array([np.cos(self._path_incline), 0, np.sin(
      self._path_incline)]) + np.array([0, 0,  - self._path_drop * (self._n_segments + 2)])
    ...
    self._goal_platform_surface_pos = np.array(
      goal_platform_pos) + np.array([0, 0, self._thickness / 2])
    ...
    max_agg_bodies = n_agent_bodies + n_puck_bodies + 3 + self._n_segments
    # 1 for table, start / goal platform, path segments
    max_agg_shapes = n_agent_shapes + n_puck_shapes + 3 + self._n_segments
    ...
    for i in range(self.n_envs):
      ...
      start_platform_actor = self.gym.create_actor(
        env_ptr, platform_asset, start_platform_start_pose, "start_platform", i, 1, 0)
      goal_platform_actor = self.gym.create_actor(
        env_ptr, platform_asset, goal_platform_start_pose, "goal_platform", i, 1, 0)
      ...
      for path_segment_idx in range(self._n_segments):
        path_segment_id = self.gym.create_actor(
          env_ptr, path_segment_asset, dummy_start_pose, f"path_segment{path_segment_idx}", i, 0, 0)
  ...
  def setup_references(self):
    ...
    for i in range(self._n_segments):
      task_handles[f"path_segment{i}"] = self.gym.find_actor_rigid_body_handle(
        self.envs[0], self._path_segment_ids[i], f"path_segment{i}")
    ...
    self._path_segment_states = self.sim_states.actor_root_states[:,
                                                                  self._path_segment_ids[0]:self._path_segment_ids[-1]+1, :]
  ...
  def _update_states(self, dt=None):
    ...
    current_target = torch.where(
      ...
      self._path_segment_states[env_ids, -1, :3] + torch.tile(torch.tensor(
        [self._path_width, 0, 0], device=self.device, dtype=torch.float32).unsqueeze(0), (self.n_envs, 1)),
      self._path_segment_states[env_ids, self._current_path_segment.clip(
        max=self._n_segments - 1), :3],
    )
  ...
  def _update_path_segments(self):
    ...
    env_ids = torch.arange(start=0, end=self.n_envs,
                           device=self.device, dtype=torch.long)
    xy_close = torch.norm(self._puck_state[:, :2] - self._path_segment_states[env_ids, self._current_path_segment.clip(
      max=self._n_segments - 1), :2], dim=-1) < (self._path_width - 0.5 * self._puck_size[env_ids].squeeze(-1)) * np.cos(self._path_incline)
    z_close = self._puck_state[:, 2] - self._path_segment_states[env_ids, self._current_path_segment.clip(
      max=self._n_segments - 1), 2] > 0.5 * self._path_width * np.sin(self._path_incline)
      ...
      if len(env_ids) > 0:
        ...
        for env_id in env_ids:
          if self._current_path_segment[env_id] != self._last_path_segment[env_id]:
            ...
            path_segment_rs_props = self.gym.get_actor_rigid_shape_properties(
              env, path_segment_id)
            path_segment_rs_props[0].friction = self._path_friction[env_id]
            self.gym.set_actor_rigid_shape_properties(
              env, path_segment_id, path_segment_rs_props)
            self.gym.set_rigid_body_color(env, path_segment_id, 0, gymapi.MESH_VISUAL, gymapi.Vec3(
              0.7, 1 - self._color_frac[env_id], self._color_frac[env_id]))
  ...
  def _reset_puck_state(self, env_ids=None):
    ...
    centered_puck_xy_state = torch.tensor(
      self._start_platform_surface_pos[:2] +
      np.array([0.375 * self._platform_depth, 0]),
      device=self.device, dtype=torch.float32)

    # Set z value, which is fixed height
    sampled_puck_state[:, 2] = self._start_platform_surface_pos[2] + \
    ...
    # Sample x y values
    noise_scale = self.start_position_noise * \
      torch.tensor([1, 10], dtype=torch.float, device=self.device).unsqueeze(0)
    sampled_puck_state[:, :2] = centered_puck_xy_state + \
      2.0 * noise_scale * \
      (torch.rand(n_resets, 2, dtype=torch.float, device=self.device) - 0.5)
  ...
  def _reset_paths(self, env_ids=None):
    ...
    # Initialize buffer to hold sampled values
    n_resets = len(env_ids)
    sampled_path_segment_location_ids_x = torch.zeros(
      n_resets, self._n_segments, device=self.device)
    sampled_path_segment_location_ids_y = torch.zeros(
      n_resets, self._n_segments, device=self.device)
    sampled_path_segment_locations = torch.zeros(
      n_resets, self._n_segments, 3, device=self.device)
    ...
    if self._path_shape == "straight":
      ...
      path_starts = torch.randint(
        low=0, high=self._path_segment_bounds[1], size=(n_resets,), device=self.device)
      ...
      path_starts = torch.where(
        ...
        torch.min(
          (ones * int(0.5 * (self._path_segment_bounds[1] - self._bend_size))).int(), path_starts),
        torch.max(
          (ones * int(0.5 * (self._path_segment_bounds[1] + self._bend_size))).int(), path_starts),
      )

      bend_row = torch.randint(
        low=1, high=self._path_segment_bounds[0]-1, size=(n_resets,), device=self.device)
      bend_direction = torch.randint(
        low=0, high=2, size=(n_resets,), device=self.device)
      ...
      bend_direction = torch.where(
        (self._path_segment_bounds[1] - path_starts <= self._bend_size) & (bend_direction > 0), -ones, bend_direction)
      bend_direction = torch.where((path_starts < self._bend_size) & (
        bend_direction < 0), ones, bend_direction)
      ...
      for i in range(self._n_segments):
        if i == 0:
          ...
          incrementer = torch.where(
            ...
            (torch.abs(
              sampled_path_segment_location_ids_y[:, i-1] - path_starts) < self._bend_size),
        ...
        sampled_path_segment_locations[:, i,
                                       0] = self._path_width / 2.0 + self._path_width * x
        sampled_path_segment_locations[:, i, 1] = (
          -self._path_bounds[1] + self._path_width) / 2.0 + self._path_width * y
        sampled_path_segment_locations[:, i, 2] = - \
          self._thickness / 2.0 - self._path_drop * (i + 1)

      # Convert xz values into global values
      c, s = np.cos(self._path_incline), np.sin(self._path_incline)
      rot_mat = torch.tensor([[c, -s], [s, c]], device=self.device,
                             dtype=torch.float).unsqueeze(0).unsqueeze(0)
      sampled_path_segment_locations[:, :, [0, 2]] = torch.matmul(
        rot_mat, sampled_path_segment_locations[:, :, [0, 2]].unsqueeze(-1)).squeeze(-1)

      # Translate the values to the global coordinates
      sampled_path_segment_locations[:, :,
                                     0] += self._start_platform_surface_pos[0] + self._platform_depth / 2.0
      sampled_path_segment_locations[:, :,
                                     2] += self._start_platform_surface_pos[2]
  ...
  def _reset_properties(self, env_ids=None):
    ...
    f_log_min, f_log_max = np.log10(self.task_cfg["path_friction"][0]), np.log10(
      self.task_cfg["path_friction"][1])
    ...
    for env_id in env_ids:
      ...
      path_frac = (np.log10(self._path_friction[env_id].item(
      )) - f_log_min) / (f_log_max - f_log_min) if f_log_max != f_log_min else 0.5
