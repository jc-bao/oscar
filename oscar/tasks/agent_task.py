...
class AgentTask(BaseTask):
  def __init__(
    ...
    self.agent = AGENT_MAPPING[agent_type](
      device=device, agent_config=self.agent_cfg)
    ...
    # How many frames to stack for observations
    self._n_frames_stack = cfg["task"]["n_frames_stack"]
    # Flag for making sure obs history only gets updated once per sim step
    self._should_update_obs_history = False
    # History of obs values (dict of deques)
  ...
  def _create_envs(self):
    ...
    self.agent_asset, self.agent_dof_props = self.agent.load_asset(
      gym=self.gym, sim=self.sim, n_envs=self.n_envs)
