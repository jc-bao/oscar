...
class Franka(Agent):
  ...
  def __init__(
    ...
    # base end effector state (shared amongst all franka variants)
    self._eef_base_state = None
  ...
  def load_asset(self, gym, sim, n_envs):
    ...
    asset = self.gym.load_asset(
      self.sim, ASSETS_ROOT, asset_file, asset_options)
    ...
    self.dof_default = to_torch(
      self.agent_config["dof_default"], device=self.device)
  ...
  def setup_references(self, sim_states, sim_actions, env_ptrs, actor_handle=0):
    ...
    if self.dof_arm_mode == gymapi.DOF_MODE_POS:
      ...
      self._arm_control = self._effort_control[:,
                                               :self.agent_config["dof_arm"]]
    else:
      raise ValueError(
        f"Invalid dof mode specified for arm, got: {self.dof_arm_mode}")
    ...
    self._eef_base_state = self._rigid_body_state[:,
                                                  self.handles["eef_base"], :]
    self._eef_base_y_state = self._rigid_body_state[:,
                                                    self.handles["eef_base_y"], :]
    self._eef_base_z_state = self._rigid_body_state[:,
                                                    self.handles["eef_base_z"], :]
    _jacobian = self.gym.acquire_jacobian_tensor(self.sim, self.name)
    jacobian = gymtorch.wrap_tensor(_jacobian)
    hand_joint_index = self.gym.get_actor_joint_dict(env_ptr, actor_handle)[
      'panda_hand_joint']
    self._j_eef = jacobian[:, hand_joint_index,
                           :, :self.agent_config["dof_arm"]]
    _massmatrix = self.gym.acquire_mass_matrix_tensor(self.sim, self.name)
    mm = gymtorch.wrap_tensor(_massmatrix)
    self._mm = mm[:, :self.agent_config["dof_arm"],
                  :self.agent_config["dof_arm"]]

    # Link jacobians
    self._j_link = jacobian[:, :self.agent_config["dof_arm"] +
                            1, :, :self.agent_config["dof_arm"]]

    # optionally history of control
    history_length = self.agent_config["history_length"]
    if history_length > 0:
      self._arm_control_history = deque([torch.zeros(self.n_envs, self.dof, device=self.device).type_as(
        self._q) for _ in range(history_length)], maxlen=history_length)
    ...
    self._link_mass = torch.zeros(
      self.n_envs, self.agent_config["dof_arm"]+1, dtype=torch.float, device=self.device)
    self._link_com = torch.zeros(
      self.n_envs, self.agent_config["dof_arm"]+1, 3, dtype=torch.float, device=self.device)
    ...
    for i, env in enumerate(env_ptrs):
      ...
      for j, (name, prop) in enumerate(zip(rigid_body_names, rigid_body_props)):
        ...
        prop.inertia.x = prop.inertia.x + \
          gymapi.Vec3(min_body_inertia[i], 0, 0)
        prop.inertia.y = prop.inertia.x + \
          gymapi.Vec3(0, min_body_inertia[i], 0)
        prop.inertia.z = prop.inertia.x + \
          gymapi.Vec3(0, 0, min_body_inertia[i])
    ...
    if self.agent_config["denormalize_control"]:
      ...
      if self.dof_arm_mode == gymapi.DOF_MODE_POS:
        u_arm = self.dof_middle[:self.agent_config["dof_arm"]].unsqueeze(0) + \
          self.dof_range[:self.agent_config["dof_arm"]
                         ].unsqueeze(0) * u_arm * 0.5
        low, high = self.dof_lower_limits.unsqueeze(
          0), self.dof_upper_limits.unsqueeze(0)
      elif self.dof_arm_mode == gymapi.DOF_MODE_VEL:
        u_arm = self.vel_limits[:self.agent_config["dof_arm"]].unsqueeze(
          0) * u_arm
        low, high = -self.vel_limits.unsqueeze(0), self.vel_limits.unsqueeze(0)
      elif self.dof_arm_mode == gymapi.DOF_MODE_EFFORT:
        u_arm = self.effort_limits[:self.agent_config["dof_arm"]].unsqueeze(
          0) * u_arm
        low, high = - \
          self.effort_limits.unsqueeze(0), self.effort_limits.unsqueeze(0)
      else:
        raise ValueError(
          f"Invalid dof mode specified for arm, got: {self.dof_arm_mode}")
  ...
  def get_control_dict(self):
    ...
    dic = {
      ...
      # [pos, quat, lvel, avel] (13 dim)
      "eef_state": self.states["eef_state"],
  ...
  def reset(self, env_ids=None):
    ...
    for env_id in env_ids:
      ...
      for j, (name, prop) in enumerate(zip(rigid_body_names, rigid_body_props)):
        ...
        prop.inertia.x = default_inertia.x + \
          gymapi.Vec3(self._min_body_inertia[env_id].item(), 0, 0)
        prop.inertia.y = default_inertia.y + \
          gymapi.Vec3(0, self._min_body_inertia[env_id].item(), 0)
        prop.inertia.z = default_inertia.z + \
          gymapi.Vec3(0, 0, self._min_body_inertia[env_id].item())
        # Also set color proportional to randomized selection if randomizing
        if self._randomize_eef_weight:
          # Set the mass for this eef
          ...
          color = gymapi.Vec3(
            1 - frac, frac, frac) if max_eef_mass_log10 != min_eef_mass_log10 else gymapi.Vec3(0.6, 0.4, 0.4)
          self.gym.set_rigid_body_color(
            env, self.handles["actor"], weight_id, gymapi.MESH_VISUAL, color)
      # Set the updated props
      self.gym.set_actor_rigid_body_properties(
        env, 0, rigid_body_props, recomputeInertia=True)
      # Modify joint properties next
  ...
  def calculate_gravity_torques(self, normalize=True):
    ...
    """
    g = torch.zeros(self.n_envs, self._dof+1, 6, 1,
                    dtype=torch.float, device=self.device)
    g[:, :, 2, :] = 9.81
    g_force = self._link_mass.unsqueeze(-1).unsqueeze(-1) * g
    # new shape is (n_envs, n_links, 7)
  ...
  def _generate_urdf(self):
    ...
    eef_link = create_link(name="panda_weight", subelements=[
                           eef_col, eef_vis], mass=mass)
    ...
    template_fpath = os.path.join(
      ASSETS_ROOT, "urdf/franka_description/robots/franka_panda_template.urdf")
    lines = []
