	...
	class Network(NetworkBuilder.BaseNetwork):
			...
			if self.is_continuous:
				...
				if self.distribution is None:
					...
					if isinstance(sigma_val, list):
						_tmp_dicts = [
							{k: v[i] if isinstance(v, list) else v for k,
							 v in self.space_config['sigma_init'].items()}
							for i in range(len(sigma_val))
						]
						sigma_init = [self.init_factory.create(
							**_tmp_dict) for _tmp_dict in _tmp_dicts]
					...
					raise ValueError(
						f"Invalid distribution specified: {self.distribution}")
		...
		def load(self, params):
			...
			if self.is_discrete or self.is_multi_discrete or not self.is_continuous:
				raise ValueError(
					"Continuous Network only supported for continuous action space!")
...
class A2CControllerBuilder(A2CBuilder):
	...
	class Network(NetworkBuilder.BaseNetwork):
			...
			if self.separate:
				...
				if self.is_continuous:
					...
					if self.differentiate_controller:
						mu = self.mu_act(self.policy_controller.get_control(
							control_dict=obs_dict['control_dict'], command=mu))
					else:
		...
		def load(self, params):
			...
			if self.is_discrete or self.is_multi_discrete or not self.is_continuous:
				raise ValueError(
					"Controller Actor-Critic only supported for continuous action space!")
	...
	class Network(nn.Module):
		...
		def forward(self, input_dict):
			...
			if is_train:
				...
				prev_neglogp = self.neglogp(
					prev_actions, prev_pre_actions, mu, sigma, logstd)
				result = {
				...
				if self.use_tanh:
					...
					selected_action, selected_pre_action = action, torch.zeros_like(
						action)     # dummy fill for pre_action
				neglogp = self.neglogp(
					selected_action, selected_pre_action, mu, sigma, logstd)
				result = {
		...
		def control_models_gradient_step(self, losses, retain_graph=False):
			...
			for name, loss in losses.items():
				self.policy_controller.learned_models[name].gradient_step(
		...
		def control_models_pre_env_step(self, control_dict, train=False, **kwargs):
			...
			ret = {}
			for model in self.policy_controller.learned_models.values():
				out = model.pre_env_step(
					control_dict=control_dict, train=train, **kwargs)
				ret.update(out)
				kwargs.update(out)
...
class A2CControllerAgent(A2CAgent):
	...
	def __init__(self, base_name, config):
		...
		"""
		print('[A2CControllerAgent] init...')
		ContinuousA2CBase.__init__(self, base_name, config)

		# Set the L2 fetch granularity
		try:
			_libcudart = ctypes.CDLL(
				'/home/pcy/miniconda3/envs/rlgpu/lib/python3.8/site-packages/torch/lib/libcudart-a7b20f20.so.11.0')
		except:
			_libcudart = ctypes.CDLL(
				'/home/pcy/anaconda3/envs/rlgpu/lib/python3.8/site-packages/torch/lib/libcudart-a7b20f20.so.11.0')
		...
		self.model.set_frozen(
			freeze=self.freeze, deterministic_when_frozen=self.config['deterministic_when_frozen'])
		...
		self.env_info['action_space'] = gym.spaces.Box(
			low=0.0, high=1.0, shape=(self.model.action_dim,))
		...
		self.optimizer = optim.Adam(self.params, float(
	...
	def init_tensors(self):
		...
		self.tensor_list += ['pre_actions', 'control_dicts',
												 'next_control_dicts', 'control_goals', 'controls']
		#
		# Get batch size
		...
		self.experience_buffer.add_tensor(
			...
			space=gym.spaces.Box(
				low=-1.0, high=1.0, shape=(self.control_dict_converter.flattened_dim,)),
		)
		...
		self.experience_buffer.add_tensor(
			name="control_goals",
			space=gym.spaces.Box(
				low=-1.0, high=1.0, shape=(self.model.policy_controller.goal_dim,)),
		)
		...
		self.experience_buffer.add_tensor(
			name="controls",
			space=gym.spaces.Box(
				low=-1.0, high=1.0, shape=(self.model.policy_controller.output_dim,)),
		)
			...
			if self.rank == 0:
				...
				if self.game_rewards.current_size > 0:
					...
					for i in range(self.value_size):
						...
						self.writer.add_scalar(
							rewards_name + '/frame'.format(i), mean_rewards[i], frame)
						self.writer.add_scalar(
							rewards_name + '/iter'.format(i), mean_rewards[i], epoch_num)
						self.writer.add_scalar(
							rewards_name + '/time'.format(i), mean_rewards[i], total_time)

					self.writer.add_scalar('episode_lengths/frame', mean_lengths, frame)
					self.writer.add_scalar('episode_lengths/iter',
																 mean_lengths, epoch_num)
	...
	def play_steps(self):
		...
		if self.last_render_time + 600 < time.time():
			print('[A2CControllerAgent] Rendering...')
			self.last_render_time = time.time()
			need_render = True
			images = self.vec_env.env.render(mode='rgb_array')
		...
		for n in range(self.steps_num):
			...
			cm_vals = self.model.control_models_pre_env_step(
				train=True, **input_dict)
	...
	def play_steps_rnn(self):
		...
		for n in range(self.steps_num):
			...
			# Add controls
			self.experience_buffer.update_data_rnn(
				'controls', indices, play_mask, self._controls)
			# Add control dicts
	...
	def preprocess_actions(self, actions):
		...
		actions = self.model.policy_action_to_control_action(
			control_dict=self.control_dict, action=actions)
	...
	def calc_gradients(self, input_dict):
		...
		with torch.cuda.amp.autocast(enabled=self.mixed_precision):
			...
			losses, _ = torch_ext.apply_masks(
				[l.unsqueeze(1) for l in control_losses.values()], rnn_masks)

			# Re-map control losses in dict
			control_losses = {name: l for name,
												l in zip(control_losses.keys(), losses)}

		# Train custom control losses
		self.model.control_models_gradient_step(
			losses=control_losses, retain_graph=False)
	...
	def get_full_state_weights(self):
		...
		extra_states = {name: model.state_dict()
										for name, model in self.model.control_models.items()}
	...
	def set_full_state_weights(self, weights, load_optimizer_state=False):
		# Run super method first
		super().set_full_state_weights(weights=weights,
		...
		for name, model in self.model.control_models.items():
			...
			if name in weights:
				...
				print(
					f"########### WARNING: {name} not found in loaded checkpoint ###########")
...
class PpoPlayerContinuousController(PpoPlayerContinuous):
	...
	def get_action(self, obs, is_deterministic=False):
		...
		current_action = self.model.policy_action_to_control_action(
			control_dict=self.control_dict, action=current_action)
...
mb.register_model('continuous_a2c_logstd_controller',
									ModelA2CContinuousLogStdController)
