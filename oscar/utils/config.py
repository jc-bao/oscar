...
def load_cfg(args, use_rlg_config=False):
  ...
  name = args.experiment_name if args.experiment_name != 'Base' else cfg[
    "policy"]["params"]["config"]['name']
  # Only save config if we're training
  if args.train:
    save_fpath = os.path.join(
      args.logdir, f"{task_name}/cfg", f"{task_name}_{controller_type}_{name}_cfg.yaml")
...
def compose_cfg(args, cfg):
  ...
  if "dof_arm_mode" in agent_cfg and agent_cfg["dof_arm_mode"] == "__AUTO__":
    ...
    if agent_cfg["dof_arm_mode"] == gymapi.DOF_MODE_POS:
      ...
      raise ValueError(
        "Only pos and effort control currently supported for robot arms!")
...
def get_args(benchmark=False, use_rlg_config=False):
  custom_parameters = [
    ...
    {"name": "--cfg_env_add", "type": str, "nargs": "+", "default": None,
     "help": "Optional additional cfg(s) to update @cfg_env. If multiple are specified, they were be processed in order"},
